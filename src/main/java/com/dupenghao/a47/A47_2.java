...
public class A47_2 {
    ...
    private static void testApplicationContext(DefaultListableBeanFactory beanFactory) throws NoSuchFieldException, IllegalAccessException {
        ...
        Map<? extends Class<?>, List<Map.Entry<Class<?>, Object>>> collect = dependencies.entrySet().stream().collect(Collectors.groupingBy(entry -> entry.getKey()));
        for (Map.Entry<Class<?>, Object> entry : dependencies.entrySet()) {
            Class<?> key = entry.getKey();
            if(key.isAssignableFrom(dd3.getDependencyType())){
