...
public class A39_2 {
    public static void main(String[] args) throws Exception{
        ...
        for (String name : names) {
            ...
            publisher.contextPrepared(context);//在spring容器创建并调用初始化器后会发起的事件
            publisher.contextLoaded(context);//所有beanDefinition加载完毕
            context.refresh();
            publisher.started(context);//spring 容器初始化完成(refresh方法调用完成)
